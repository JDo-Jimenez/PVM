...
int main(int argc,char* argv[],char **envp)
{
  ...
  // cuantas lineas me tocan a mi (redondeado por arriba)
  {
    ...
    inicial= a*un_id; // Que línea es la primera que debo calcular:

    if(b){
      // no es un divisor exacto;
      if(b>un_id){
	mias=mias+1;
	inicial= inicial + un_id; // las que han consumido de más los anteriores
      }else{                                    					
	//Se encargan los anteriores
	inicial= inicial + b; // las que han consumido de más los anteriores              
      }
